...
const isProd = process.env.NODE_ENV === 'production'; // npm run build sets NODE_ENV=production
const destFolder = isProd ? 'build' : 'dist';
// always use assets folder for CSS, JS, images, fonts
...
export function vendorsJs() {
  if (!paths.vendors.js.length) return Promise.resolve();
...
// ==================== BUILD & DEV ====================
export const build = gulp.series(clean, gulp.parallel(styles, vendorsCss, scripts, vendorsJs, images, fonts, html));
